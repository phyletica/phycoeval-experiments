\documentclass[border=10pt,varwidth=30cm]{standalone}
\usepackage{graphicx,capt-of}
\usepackage{subcaption}
\usepackage[cm]{sfmath}
\usepackage[T1]{fontenc}

\usepackage{adjustbox,xcolor}

\definecolor{shadecolor}{gray}{0.9}

\renewcommand{\familydefault}{\sfdefault}

\newcounter{subfloat}
\renewcommand{\thesubfloat}{\Alph{subfloat}}
\newcommand{\insertlabel}{%
    \huge%
    \stepcounter{subfloat}%
    \textbf{\thesubfloat}}

\DeclareCaptionSubType[Alph]{figure}
\captionsetup[subfigure]{labelformat=simple,labelsep=space}

\begin{document}

\begin{figure}
    \centering
    \begin{subfigure}[c][][c]{0.44\textwidth}
        \centering
        \Huge $M_{G}$ = \textbf{Generalized model}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c][][c]{0.54\textwidth}
        \centering
        \Huge $M_{IB}$ = \textbf{Independent-bifurcating model}
    \end{subfigure}

    % \vspace{2mm}
    \begin{subfigure}[c][][c]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../legend-horizontal-short-sans-cropped.pdf}
    \end{subfigure}

    \vspace{3mm}
    \begin{subfigure}[b][][l]{0.55\textwidth}
        \insertlabel
    \end{subfigure}
    \hfill
    \begin{subfigure}[b][][l]{0.43\textwidth}
        \insertlabel
    \end{subfigure}

    \vspace{2mm}
    \begin{subfigure}[c][][c]{0.53\textwidth}
        \begin{adjustbox}{bgcolor=shadecolor}
        \centering
        \includegraphics[width=\textwidth]{../../tex/trees/bifurcating-tree-tall-cropped.pdf}
        \label{fig:gentree}
        \end{adjustbox}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c][][c]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../fixed-bif-true-topo-probs-sans-cropped.pdf}
    \end{subfigure}

    \vspace{5mm}
    \begin{subfigure}[b][][l]{\textwidth}
        \insertlabel
    \end{subfigure}

    \vspace{2mm}
    \begin{subfigure}[c][][c]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../fixed-bif-gen-wrong-probs-sans-cropped.pdf}
    \end{subfigure}

    \begin{subfigure}[b][][l]{\textwidth}
        \insertlabel
    \end{subfigure}

    \begin{subfigure}[c][][c]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../fixed-bif-euclidean-distances-large-sans-cropped.pdf}
    \end{subfigure}
\end{figure}

\end{document}
