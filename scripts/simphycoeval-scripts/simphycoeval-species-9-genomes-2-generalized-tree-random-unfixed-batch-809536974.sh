...
rng_seed=809536974
number_of_reps=10
sim_name="species-9-genomes-2-generalized-tree-random-unfixed"
config_dir="../../configs"
config_path="${config_dir}/species-9-genomes-2-generalized-tree-random.yml"
...
$exe_path --seed="$rng_seed" -n "$number_of_reps" -t "$number_of_topo_mcmc_gens_per_rep" -o "$output_dir" -p "${config_dir}/species-9-genomes-2-bifurcating-tree-random.yml" -p "${config_dir}/species-9-genomes-2-generalized-tree-random.yml" "$config_path" && julia --project "$config_set_up_script_path" "$output_dir" && julia --project "$qsub_set_up_script_path" "$output_dir"
