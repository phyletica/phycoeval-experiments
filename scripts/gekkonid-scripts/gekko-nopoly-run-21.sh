...
new_config_path="${output_dir}/${config_file_prefix}.yml"
out_path="${prefix}${config_file_prefix}.out"

if [ ! -e "$new_config_path" ]
then
    sed -e "s/starting_tree: comb/starting_tree: random/g" "$config_path" > "$new_config_path"
fi

"$exe_path" --seed $run --nthreads "$nthreads" --prefix $prefix --relax-triallelic-sites --relax-missing-sites "$new_config_path" 1>"$out_path" 2>&1
