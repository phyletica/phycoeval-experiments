...
struct SimResults
    ...
    function SimResults(; suffix = "",
        ...
        results = DataFrame()
        for is_fixed in is_fixed_conditions
            for sim_cond in sim_conditions
                for locus_size in locus_sizes
                    ...
                    for locus_condition in locus_conditions
                        sim_dir_name = ""
                        if is_fixed
                            sim_dir_name = "$(locus_condition)species-9-genomes-2-$sim_cond-tree-provided-fixed"
                        else
                            sim_dir_name = "$(locus_condition)species-9-genomes-2-$sim_cond-tree-random-unfixed"
                        end
                        sim_dir = joinpath(ProjectUtil.SIM_DIR, sim_dir_name)
                        for batch_dir in ProjectUtil.batch_dir_iter(sim_dir)
                            for analysis_cond in analysis_conditions
                                ...
                                for is_var_only in var_only_bools
                                    ...
                                    df[!, "sim_generalized"] = repeat([sim_cond == "generalized"], nrows)
                                    df[!, "analysis_generalized"] = repeat([analysis_cond == "generalized"], nrows)
...
function get_violin_plot(
    ...
    if isnothing(marker_shapes)
        marker_shapes = fill(:circle, 1, length(values))
    end
    if isnothing(marker_sizes)
        marker_sizes = fill(4.0, 1, length(values))
    end
...
function make_legends()
    ...
    Plots.plot!(plt,
            ...
            markerstrokealpha = 0.0)
    annotate!(plt, 0.03, 0,
              text(LaTeXString("$(gen_model) = Generalized model"),
                   :left,
                   :vcenter,
                   8),
              annotation_clip = false)
    annotate!(plt, 0.83, 0,
              text(LaTeXString("$(bif_model) = Independent-bifurcating model"),
                   :left,
                   :vcenter,
                   8),
              annotation_clip = false)
    Plots.plot!(plt, size = (250, 80), grid = false, ticks = false)
    Plots.xaxis!(plt, false)
    Plots.yaxis!(plt, false)
    plot_path = joinpath(ProjectUtil.RESULTS_DIR, "legend-horizontal-all-sites.tex")
    Plots.savefig(plt, plot_path)
    process_tex(plot_path, target = axis_pattern, replacement = axis_replace)
    return nothing
end
...
function main_cli()::Cint
    ...
    for locus_size in locus_sizes
        ...
        bif_gen_wrong_merged_heights = get_rows(merged_target_height_results,
                false,
                false,
                true,
                false,
        ...
        plot_path = joinpath(ProjectUtil.RESULTS_DIR, "$(locus_prefix)unfixed-bif-gen-wrong-merged-height-probs.tex")
        ...
        Plots.ylabel!(p, "Posterior probability")
        Plots.xlabel!(p, "Time difference")
        plot_path = joinpath(ProjectUtil.RESULTS_DIR, "$(locus_prefix)unfixed-bif-gen-var-only-wrong-merged-height-probs-scatter.tex")
        Plots.savefig(p, plot_path)
        process_tex(plot_path, target = axis_pattern, replacement = axis_replace)
